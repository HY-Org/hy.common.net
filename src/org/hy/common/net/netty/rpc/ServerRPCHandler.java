...
public class ServerRPCHandler extends SimpleChannelInboundHandler<Data>
{
    ...
    private static final Map<ChannelHandlerContext ,Object> $Clients = new Hashtable<ChannelHandlerContext ,Object>();
    ...
    {
        Object v_Client = $Clients.get(i_Ctx);
        ...
        {
            writeResponse(i_Ctx ,this.mainActive(i_Ctx ,i_Msg));
        }
    }
     ...
     * 2. 当允许游客登录访问时，即 ServerRPC.Validate 为空值时，只要求 LoginRequest.name 有值即可
     * 
     * @author      ZhengWei(HY)
     * @createDate  2021-09-25
     * @version     v1.0
    ...
    private LoginResponse.Builder login(final ChannelHandlerContext i_Ctx ,final Data i_Msg)
    {
        ...
        if ( Data.DataType.LoginRequest == i_Msg.getDataType() )
        {
            ...
            if ( v_LoginRet )
            {
                v_LoginRespBuilder.setPost(this.mainServer.getPort());
                
                $Clients.put(i_Ctx ,i_Msg.getLoginRequest().getUserName());
            }
        }
        else
        {
     ...
     * @return
     */
    private Response.Builder mainActive(final ChannelHandlerContext i_Ctx ,final Data i_Msg)
    {
        ...
        {
            return ObjectToProto.toResponse(execute(v_Listener ,v_CRequest));
        }
        // 异步线程处理机制
        else
        {
            ...
            this.mainServer.getExecutorPool().submit(new Callable<Object>()
            {
                ...
                {
                    writeResponse(i_Ctx ,ObjectToProto.toResponse(execute(v_FListener ,v_FRequest)));
                    return null;
                }
            });
        }
     ...
     */
    private CommunicationResponse execute(CommunicationListener i_Listener ,CommunicationRequest i_Request)
    {
